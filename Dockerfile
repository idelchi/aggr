...
COPY --parents **/*.go go.mod go.sum ./
ARG AGGR_VERSION="unofficial & built by unknown"
RUN --mount=type=cache,target=${GOMODCACHE},uid=${UID},gid=${UID},id=gomod-${TARGETARCH} \
    ...
    GOOS=${TARGETOS} GOARCH=${TARGETARCH} CGO_ENABLED=0 go build -ldflags="-s -w -X 'main.version=${AGGR_VERSION}'" -o bin/ .
...
RUN cp bin/aggr /home/${USER}/.local/bin
...
RUN mkdir -p /home/${USER}/.aggr
COPY --chown=${USER}:${USER} . /home/${USER}/.aggr
